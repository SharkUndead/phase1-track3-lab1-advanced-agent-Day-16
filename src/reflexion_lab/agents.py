...
class BaseAgent:
    ...
    def run(self, example: QAExample) -> RunRecord:
        ...
        for attempt_id in range(1, self.max_attempts + 1):
            ...
            # Reflexion logic: only for reflexion agent, only when attempts remain
            if self.agent_type == "reflexion" and attempt_id < self.max_attempts:
                reflection, ref_tokens = reflector(example, attempt_id, answer, judge)
                # Store BOTH lesson and next_strategy so Actor has full context
                reflection_memory.append(reflection.lesson)
                reflection_memory.append(reflection.next_strategy)
                reflections.append(reflection)
                
                # Add reflector token usage to this attempt
